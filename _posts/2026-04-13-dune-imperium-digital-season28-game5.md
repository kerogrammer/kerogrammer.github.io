...
> 시즌 28, 68점(543위). 게임 4의 여운이 채 가시기도 전에 **게임 5**가 시작되었다. 이번 판은 처음부터 무언가 달랐다.
...
이번 판의 핵심 키워드는 **「우주 정치학」**이다. 규칙 모디파이어가 "팩션 동맹 체결 시 솔라리를 추가 지급"하도록 바꾸자, 테이블 위의 역학 관계가 완전히 뒤집혔다.

일반적인 게임에서는 동맹보다 전투에서 승점을 죄어모으는 게 우선이지만, 이번 판에서는 **팩션 영향력을 빠르게 올려 동맹을 먼저 맺는 것**이 솔라리 수급과 직결된다. 결국 네 명의 지도자 모두가 동맹에 온 신경을 집중하면서 게임 속도가 비정상적으로 빨라졌고, 예정된 10라운드가 아닌 **단 7라운드 만에 승점 10점을 돌파한 지도자가 나와** 게임이 종료되었다.

순위표 점수 모디파이어(**조달**, **전설**) 또한 덱 구성에 직접적인 영향을 미쳤지만, 이번 판은 이것들을 챙길 여유조차 없을 만큼 정치전이 치열하게 펼쳐졌다.
...
<div class="img-caption">헬레나 리체스의 시대는 잠시 멈추고, 이번 판에는 아트레이드 가문의 공작이 나선다.</div>

이번 판은 평소 즐겨 쓰던 **헬레나 리체스**가 아닌, **레토 아트레이드 공작**을 선택했다. 이유는 단 하나, 그의 **인장 반지(Signet Ring) 능력** 때문이다.

> *"스파이스 1개를 지불하면, 나보다 영향력이 높은 팩션에서 영향력 1을 획득한다."*

「우주 정치학」이 동맹에 솔라리 보너스를 얹어주는 판에서, 스파이스 한 조각으로 뒤처진 팩션 영향력을 따라잡을 수 있는 이 능력은 **구조적 우위**를 선점하게 해준다. 빠르게 여러 팩션 동맹을 맺을수록 솔라리가 쌓이고, 쌓인 솔라리로 다시 좋은 카드를 구매해 선순환을 만드는 것이 이번 판의 핵심 전략이었다.
...
손패에서 **「인장 반지」**가 보이자마자 전략 방향이 확정되었다. 카드를 보드로 내면서 인장 반지 능력을 즉시 발동, 나보다 영향력이 높은 팩션 두 곳에서 영향력을 각각 1씩 끌어올렸다.
...
1라운드 전투는 과감히 포기했다. 일반 리체스 백작이 전투력 **8점**으로 압도적 1위를 가져갔고, 나는 영향력 쌓기에 집중했다. 이건 손해가 아니다 — **미래의 동맹 솔라리를 위한 투자**다.
...
<div class="img-caption">라운드 2 · 분쟁: 카르타그 공성전 — 현재 스파이스 0, 솔라리 7 보유</div>

2라운드는 솔라리 7개라는 두둑한 자원으로 시작했다. 분쟁은 **카르타그 공성전**. 이번에도 전투보다는 카드 구매와 영향력에 집중했다.
...
테시아 버니우스가 전투력 **10점**의 괴력으로 카르타그를 차지했다. 내가 노린 건 전투가 아니라 카르타그 전투 도중 「인장 반지」를 한 번 더 활용해 쌓아올린 팩션 영향력이었다.
...
3라운드, 분쟁은 **사막의 힘**. 여기서 그동안 모아온 자원과 전투력을 처음으로 쏟아부었다.
...
<div class="dune-callout">
  ...
  보상으로 <strong>솔라리 + 물</strong>을 챙기며, 누적 점수가 오르기 시작했다.
</div>
...
4라운드에서 제국열에 등장한 것은 **「배반」** 카드다. 영향력을 **2점**씩 올려주고 스스로 폐기되는 이 카드는, 팩션 동맹을 노리는 이번 판에서 황금같은 카드였다. 단번에 영향력 두 단계를 끌어올릴 수 있다는 것은 동맹 체결 타이밍을 대폭 앞당겨준다.
...
일반 리체스 백작이 전투력 **9점**으로 1위를 가져갔다. 나는 2위(전투력 5점)로 **물 1개 + 승점 1점**을 획득.
...
<div class="img-caption">라운드 5 · 분쟁: 길드 은행 습격 — 스파이스 8, 솔라리 0. 「배반」 카드가 손패에 들어왔다.</div>

5라운드, **「배반」** 카드가 드디어 내 손에 들어왔다. 영향력 2점을 한 번에 올릴 수 있는 기회다.
...
전투력 4점씩으로 나와 헌드로 모리타니가 **공동 2위**를 기록했다. 테시아 버니우스가 전투력 2점으로 오히려 최하위. 흥미로운 점은 1위가 없었다는 것 — 즉 최대 보상은 아무도 가져가지 못했다.
...
6라운드의 분쟁은 **독점 거래**. 이번 판부터 전투에 더 공격적으로 임하기 시작했다.

7라운드 직전 상황에서 **「초머르키」** 기술 카드를 획득했다(17-round6-result의 배경 화면). 이 카드는 종료 단계에서 동점자 대결 시 내가 승리하는 기술 — 후반 접전을 대비한 포석이었다.

**6라운드 결과:**

독점 거래 전투에서 **전투력 9점으로 1위**를 차지하며 솔라리와 공개 카드 2장을 획득했다. 누적 승점이 빠르게 치고 올라가고 있었다.
...
<div class="dune-callout">
  <strong>「우주 정치학」의 공포:</strong> 팩션 동맹마다 솔라리가 굴러들어오자 모든 지도자들이 영향력에 미쳐 달렸다. 그 결과, 테시아 버니우스가 7라운드에 <strong>승점 10점</strong>을 넘기며 게임 종료 트리거가 발동되었다.
</div>
...
<div class="img-caption">접전 중 공개된 「거대한 음모」 —조건부 종료 단계 VP 카드. 테시아 버니우스가 이미 부분 달성 상태였다.</div>

![7라운드: 스파이스는 흘러야 한다](/assets/img/posts/dune-imperium-digital-season28-game5/21-round8-combat.webp)
<div class="img-caption">제국열에 「스파이스는 흘러야 한다」 (비용 9, 승점 1) 등장. 다른 지도자들이 달려들었다.</div>

7라운드의 분쟁 명칭은 **카르타그 대전투**. 전장이 최고조로 달아올랐다.
...
마지막 전투에서 손패의 **「배반」**을 포함한 모든 자원을 쏟아부어 **전투력 14점** 투입. 동시에 덱에 「거대한 음모」 달성 진행 상황도 확인하며 종료 단계 보너스를 최대로 끌어올렸다.
...
마지막 전투는 테시아 버니우스가 전투력 **19점**이라는 경이로운 수치로 1위를 차지했다. IRIS2025는 2위(14점)로 아쉬웠지만, **승점 면에서는 전혀 다른 이야기**가 펼쳐졌다.
...
**전투에서는 테시아가 마지막 라운드를 지배했지만, 승점에서는 내가 여유 있게 앞섰다.** 팩션 영향력 선점과 동맹 솔라리 수급, 카드 구매로 이어진 선순환 전략이 정확히 맞아떨어진 결과였다.
...
게임 4에서 틀레이락스 카드를 차곡차곡 쌓아 후반 폭발력으로 10라운드를 가져가는 전략을 썼다면, **게임 5는 완전히 다른 결의 판이었다.**

「우주 정치학」은 단순히 솔라리를 더 주는 규칙이 아니었다. **모든 지도자의 플레이 스타일을 강제로 바꿔버리는** 메타 변환이었다. 결과적으로 게임이 7라운드로 단축되었고, 이는 곧 "후반에 터지는 빌드"가 아닌 "초반부터 효율적으로 선점하는 자"가 유리한 판이었다는 뜻이다.

그 점에서 **레토 아트레이드 공작의 인장 반지**는 이번 모디파이어와 완벽한 궁합을 보였다. 스파이스 한 개로 영향력을 추격할 수 있다는 능력은, 모든 지도자가 팩션 영향력 경쟁에 몰두하는 판에서 **비용 효율 최강의 카드**였다.

**전설** 모디파이어를 위한 네임드 캐릭터 수집은 한 장도 달성하지 못했던 것이 아쉬운 점. 다음 판에서는 조달과 전설 보너스도 챙길 수 있는 지도자와 전략을 고민해볼 것이다.

> 시즌 28, 누적 89점. 494위. 아라키스에서의 전쟁은 계속된다. 다음 게임에서도 좋은 성적으로 돌아오겠다. 🏜️
